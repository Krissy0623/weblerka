...
switch ($op){
  case "order_list" :
    ...
    redirect_header($returnUrl, "訂購成功", 2000);    
		exit; 
...
function order_insert($sn=""){
  ...
  $_POST['tel'] = db_filter($_POST['tel'], '');
  $_POST['email'] = db_filter($_POST['email'], '');
  $_POST['kind_sn'] = db_filter($_POST['kind_sn'], '配送方式');
...
function op_list(){
  ...
  $sql = "SELECT a.sn,a.title,a.content,a.price,
  ...
  while($row = $result->fetch_assoc()){    
    ...
    $row['price'] = (int)$row['price'];//價格
    $row['prod'] = getFilesByKindColsnSort("prod",$row['sn']);    
    // $row['content'] = htmlspecialchars($row['content']);  //內容不用過濾--此行不用過濾
