...
-- 畫畫資料表
CREATE TABLE `draws` (
    `sn` int(10) unsigned NOT NULL auto_increment COMMENT 'draws_sn',
    `title` varchar(255) NOT NULL default '' COMMENT '主題',
    `content` text NULL COMMENT '內容',
    `enable` enum('1','0') NOT NULL default '1' COMMENT '狀態',
    `date` int(10) unsigned NOT NULL default 0 COMMENT '建立日期',
    `sort` smallint(5) unsigned NOT NULL default 0 COMMENT '排序',
    PRIMARY KEY  (`sn`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COMMENT='畫畫資料表';